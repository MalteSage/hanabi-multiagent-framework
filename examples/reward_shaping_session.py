    ...
    if agent_type.type == 'rainbow':
        ...
        if agent_params.pbt is True:
            ...
            return DQNAgent(env.observation_spec_vec_batch()[0],
                            ...
                            agent_params,
                            reward_shaper,
                            [sample_buffersize(pbt_params, agent_params) for n in range(agent_params.n_network)],
                            [sample_init_lr(pbt_params) for n in range(agent_params.n_network)],
...
def split_evaluation(total_reward, n_network, prev_rew):
    ...
    print('Splitting evaluations for {} states and {} agents!'.format(len(total_reward), n_network))
...
#             setattr(opt_states, field, FlatMapping(obj)) 
    ...
    for pair in couples:
        ...
        if 'buffersize' in hyperparams:
            ...
            agent.buffer[index_loser[pair[0]]].change_size(buffersizes[index_survivor[pair[1]]])
        ...
        if 'alpha' in hyperparams:
            ...
            agent.alpha = alphas
            agent.buffer[index_loser[pair[0]]].alpha = alphas[index_survivor[pair[1]]]
...
def session(
    ...
    mean_reward = split_evaluation(total_reward, agent_params.n_network, mean_reward_prev)
